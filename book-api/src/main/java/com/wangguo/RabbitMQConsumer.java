...
public class RabbitMQConsumer {
    ...
    // 监听消息队列中的消息,@RabbitListener注解指定目标方法来作为消费消息的方法，通过注解参数指定所监听的队列或者Binding。使用@RabbitListener可以设置一个自己明确默认值的RabbitListenerContainerFactory对象。
    @RabbitListener(queues = {RabbitMQConfig.QUEUE_SYS_MSG}) // 被这个注解注解的方法可以用来消费消息
    public void watchQueue(String payload, Message message) {
        ...
        if (routingKey.equalsIgnoreCase("sys.msg." + MessageEnum.FOLLOW_YOU.enValue)) {
            ...
            msgService.createMsg(messageMO.getFromUserId(),
                    messageMO.getToUserId(),
                    MessageEnum.COMMENT_VLOG.type, messageMO.getMsgContent());
            ...
            msgService.createMsg(messageMO.getFromUserId(),
                    messageMO.getToUserId(),
                    MessageEnum.REPLY_YOU.type, messageMO.getMsgContent());
        } else if (routingKey.equalsIgnoreCase("sys.msg" + MessageEnum.LIKE_COMMENT.enValue)) {
            msgService.createMsg(messageMO.getFromUserId(),
                    messageMO.getToUserId(),
                    MessageEnum.LIKE_VLOG.type, messageMO.getMsgContent());
        } else {
